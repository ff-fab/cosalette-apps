...
| -------------------------------------- | ---------------------------------- | ------ | --- |
| `vito2mqtt/_meta/registry`            | Canonical AsyncAPI 3.0.0 document   | yes    | 1   |
| `vito2mqtt/_meta/state_model_drift`   | `state_model` drift snapshot JSON   | yes    | 1   |

`_meta/registry` is the canonical AsyncAPI document describing every channel
vito2mqtt publishes and subscribes to; inbound command channels are stripped from the
published copy so the command surface is not exposed to anyone who can subscribe on a
shared broker.
