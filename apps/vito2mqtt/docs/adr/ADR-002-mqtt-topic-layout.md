...
!!! note "Editorial note (2026-09-19)"
    The unused `device_id` setting is removed (cap-ohpx). The `{device}` segment in the examples of Option 2 and Option 3 was illustrative and is not part of any layout that the app publishes.

!!! note "Editorial note (2026-09-19)"
    Two instances on one broker publish to the same topics. To separate them, set a different `VITO2MQTT_MQTT__TOPIC_PREFIX` for each instance.
