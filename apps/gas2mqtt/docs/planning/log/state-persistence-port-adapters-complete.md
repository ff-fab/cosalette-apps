...
`NullStorage`, `FakeStorage`) following the existing hexagonal architecture pattern. All
16 unit tests pass, including mutation isolation tests for `FakeStorage`.
...
- `packages/src/gas2mqtt/adapters/fake.py` (modified — added `NullStorage`,
  `FakeStorage`)
...
- `FakeStorage.__init__()`, `.load()`, `.save()` — in-memory test double with deep-copy
  isolation
