...
class TestDiscoveryOptOut:
    ...
    def test_opted_out_channels_are_exactly_the_documented_set(
        ...
        - ``consumptionCommand``: operator control surface for resetting
          the meter reading.
        - ``consumptionState``: command acknowledgement, not a datapoint.
        - ``magnetometerState``: raw three-axis field readings, a debug aid
          for tuning pulse detection.
        """
        opted_out = {
            name
